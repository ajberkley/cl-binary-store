...
(declaim (inline store-simple-string))
(defun store-simple-string (string storage &optional references)
  ...
  (labels ((write-it ()
	     (with-write-storage (storage)
	       ...
	       (let* ((string-length (length string))
		      (num-bytes (the fixnum (* 4 string-length))))
		 (store-tagged-unsigned-fixnum string-length storage)
		 (let ((offset (ensure-enough-room-to-write storage num-bytes)))
		   (with-storage-sap (sap storage)
		     (sb-sys:with-pinned-objects (string)
		       (copy-sap sap offset (sb-sys:vector-sap string) 0 num-bytes)))
...
(declaim (inline restore-simple-string))
(defun restore-simple-string (storage)
  (declare (optimize speed safety))
  (let* ((length (restore-object storage nil))
	 (num-bytes (the fixnum (* 4 (the fixnum length))))
